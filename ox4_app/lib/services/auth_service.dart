...
class AuthService {
  ...
  static Future<Map<String, dynamic>?> criarCorrida(Map<String, dynamic> data) async {
    if (useMock) return {'id': 123, 'status': 'PENDENTE'};
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('access_token');
      final response = await http.post(
        Uri.parse('${ApiConfig.baseUrl}/corridas/'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
      ...
      print('Erro ao criar corrida: ${response.statusCode} - ${response.body}');
      return null;
    } catch (e) {
      print('Exceção ao criar corrida: $e');
      return null;
    }
  }

  static Future<Map<String, dynamic>?> getStatusCorrida(int id) async {
    if (useMock) return {'status': 'ACEITE', 'motorista': {'nome': 'Carlos Silva', 'telefone': '923111222'}};
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('access_token');
      final response = await http.get(
        Uri.parse('${ApiConfig.baseUrl}/corridas/$id/'),
        headers: {'Authorization': 'Bearer $token'},
      );
      if (response.statusCode == 200) {
        return json.decode(response.body);
      ...
      print('Erro ao obter status da corrida: $e');
      return null;
    }
  }
}
